...
int main(int argc, char *argv[]) {
    ...
    char *project_upper = ut_strdup(project);
    strupper(project_upper);

    ut_init(argv[0]);
    ut_load_init(NULL, NULL, NULL, NULL);

    files_parsed = ut_rb_new(compare_string, NULL);

    const char *project_path = ut_locate(project, NULL, UT_LOCATE_DEVSRC);
    if (!project_path) {
        ut_error("cannot find source for project '%s'", project);
    } else {
        ut_info("amalgamate '%s' => '%s'", project, project_path);
    }
    ...
    ut_try(amalgamate(header_out, include_path, true, include_file), NULL);
    fclose(header_out);
    ...
    while (ut_iter_hasNext(&it)) {
        ...
        ut_try(amalgamate(src_out, include_path, false, file_path), NULL);
        free(file_path);
    } 
